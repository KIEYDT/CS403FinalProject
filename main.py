...
def MainPage():
    ...
    # จัดการผู้ค้าส่ง
    supplierManagementBtn = Button(menuFrame, text="แก้ไข/บันทึกข้อมูลผู้ค้าส่ง", bg="gray", fg="white", borderless=1,
                                   command=supplierManagementClicked)
    supplierManagementBtn.grid(row=1, columnspan=2, sticky="news")

    # จัดการข้อมูลการสั่งซื้อ
    purchaseManagementBtn = Button(menuFrame, text="เพิ่ม/แก้ไขข้อมูลการสั่งซื้อ", bg="gray", fg="white", borderless=1,
                                   command=purchaseManagementClicked)
    ...
    # ตรวจสอบข้อมูล
    intelInvestigateBtn = Button(menuFrame, text="ตรวจสอบข้อมูลการสั่งซื้อ", fg="white", bg="gray",
                                 command=intelinvestigateClicked, borderless=1)
    ...
    # Logout Button
    Button(menuFrame, text="Log out", bg="white", fg="black", borderless=1, command=mainpageFrame.destroy).grid(row=9,
                                                                                                                column=0)
    # Exit Button
    Button(menuFrame, text="Exit", bg="red", fg="black", borderless=1, command=exit).grid(row=9, column=1)

    btnlist = [warehouseBtn, supplierManagementBtn, productSearchBtn, intelInvestigateBtn, saveIntelBtn, printReportBtn,
               purchaseManagementBtn]
...
def warehouseClicked():
    ...
    warehouseTree["columns"] = (
    "รหัสสินค้า", "ชื่อสินค้า", "ราคาต้นทุนต่อหน่วย", "ราคาขายต่อหน่วย", "ปริมาณคงเหลือ", "วันที่บันทึกล่าสุด")
    ...
    # Add Button
    Button(middle, text="เพิ่มสินค้า", fg="black", bg="green", borderless=1, command=addProductClicked).grid(row=0,
                                                                                                             column=0,
                                                                                                             sticky='ne',
                                                                                                             padx=60,
                                                                                                             pady=170)
    ...
    # Modify Button
    Button(middle, text="แก้ไขสินค้า", fg="black", bg="yellow", borderless=1, command=modifyProductClicked).grid(row=0,
                                                                                                                 column=0,
                                                                                                                 sticky='se',
                                                                                                                 padx=160,
                                                                                                                 pady=170)
    ...
    # Delete Button
    Button(middle, text="ลบสินค้า", fg="black", bg="red", borderless=1, command=deleteProductClicked).grid(row=0,
                                                                                                           column=0,
                                                                                                           sticky='se',
                                                                                                           padx=60,
                                                                                                           pady=170)
...
def addProductClicked():
    def AddProduct():
        if prodnameSpy.get() == "":
            ...
            if isValid:
                ...
                cursor.execute(sql,
                               [prodnameSpy.get(), prodCostEnt.get(), prodPriceEnt.get(), prodQuantityEnt.get(), d])
                conn.commit()
    ...
    Label(addProductFrame, text="ชื่อสินค้า:", fg="black", bg="white", font="verdana 25").grid(row=0, column=0,
                                                                                               sticky='e')
    prodnameEnt = Entry(addProductFrame, textvariable=prodnameSpy, width=20)
    prodnameEnt.grid(row=0, column=1, sticky='w')

    Label(addProductFrame, text="ราคาต้นทุน:", fg="black", bg="white", font="verdana 25").grid(row=1, column=0,
                                                                                               sticky='e')
    ...
    Label(addProductFrame, text="ปริมาณในคลัง:", fg="black", bg="white", font="verdana 25").grid(row=3, column=0,
                                                                                                 sticky='e')
    prodQuantityEnt = Entry(addProductFrame, width=20)
    prodQuantityEnt.grid(row=3, column=1, sticky='w')

    Button(addProductFrame, text="เพิ่มสินค้า", fg="black", bg="green", borderless=1, font="verdana 25 bold",
           command=AddProduct).grid(row=4, columnspan=2)
...
def modifyProductClicked():
    ...
    if warehouseTree.focus() == "":
        ...
        Label(modifyFrame, text="ชื่อสินค้า:", fg="black", bg="white", font="verdana 25").grid(row=0, column=0,
                                                                                               sticky='e')
        prodnameEnt = Entry(modifyFrame, textvariable=prodnameSpy, width=20)
        prodnameEnt.grid(row=0, column=1, sticky='w')

        Label(modifyFrame, text="ราคาต้นทุน:", fg="black", bg="white", font="verdana 25").grid(row=1, column=0,
                                                                                               sticky='e')
        ...
        Label(modifyFrame, text="ปริมาณในคลัง:", fg="black", bg="white", font="verdana 25").grid(row=3, column=0,
                                                                                                 sticky='e')
        prodQuantityEnt = Entry(modifyFrame, width=20)
        prodQuantityEnt.insert(END, selectedProd_quantity)
        prodQuantityEnt.grid(row=3, column=1, sticky='w')

        Button(modifyFrame, text="แก้ไขสินค้า", fg="black", bg="yellow", borderless=1, font="verdana 25 bold",
               command=ModifyProduct).grid(row=4, columnspan=2)
...
def supplierManagementClicked():
    ...
    # header
    Label(top, text="แก้ไข/บันทึกผู้ค้าส่ง", fg="black", bg="white", font="verdana 35 bold").grid(row=0, column=0,
                                                                                                  sticky='s')
    ...
    # Add Button
    Button(middle, text="เพิ่มผู้ค้าส่ง", fg="black", bg="green", borderless=1, command=addSupplierClicked).grid(row=0,
                                                                                                                 column=0,
                                                                                                                 sticky='ne',
                                                                                                                 padx=180,
                                                                                                                 pady=170)
    ...
    # Modify Button
    Button(middle, text="แก้ไขผู้ค้าส่ง", fg="black", bg="yellow", borderless=1, command=modifySupperlierClicked).grid(
        row=0, column=0, sticky='se', padx=280, pady=170)

    # Delete Button
    Button(middle, text="ลบผู้ค้าส่ง", fg="black", bg="red", borderless=1, command=deleteSupplierClicked).grid(row=0,
                                                                                                               column=0,
                                                                                                               sticky='se',
                                                                                                               padx=180,
                                                                                                               pady=170)
...
def addSupplierClicked():
    def AddSupplier():
        ...
        elif phoneEnt.get().replace('-', '').isnumeric() == False or (
                len(phoneEnt.get().replace('-', '')) != 10 and len(phoneEnt.get().replace('-', '')) != 9):
    ...
    Label(addSupplierFrame, text="ชื่อร้าน/สวน:", fg="black", bg="white", font="verdana 25").grid(row=0, column=0,
                                                                                                  sticky='e')
    supplierNameEnt = Entry(addSupplierFrame, width=20)
    supplierNameEnt.grid(row=0, column=1, sticky='w')

    Label(addSupplierFrame, text="ชื่อผู้ติดต่อ:", fg="black", bg="white", font="verdana 25").grid(row=1, column=0,
                                                                                                   sticky='e')
    contactNameEnt = Entry(addSupplierFrame, width=20)
    contactNameEnt.grid(row=1, column=1, sticky='w')

    Label(addSupplierFrame, text="เบอร์โทรศัพท์ติดต่อ:", fg="black", bg="white", font="verdana 25").grid(row=2,
                                                                                                         column=0,
                                                                                                         sticky='e')
    phoneEnt = Entry(addSupplierFrame, width=20)
    phoneEnt.grid(row=2, column=1, sticky='w')

    Button(addSupplierFrame, text="เพิ่มผู้ค้าส่ง", fg="black", bg="green", borderless=1, font="verdana 25 bold",
           command=AddSupplier).grid(row=3, columnspan=2)
...
def modifySupperlierClicked():
    ...
    if supplierTree.focus() == "":
        ...
        Label(modifyFrame, text="ชื่อร้าน/สวน:", fg="black", bg="white", font="verdana 25").grid(row=0, column=0,
                                                                                                 sticky='e')
        supplierNameEnt = Entry(modifyFrame, width=20)
        supplierNameEnt.insert(END, selectedSupp_name)
        supplierNameEnt.grid(row=0, column=1, sticky='w')

        Label(modifyFrame, text="ชื่อผู้ติดต่อ:", fg="black", bg="white", font="verdana 25").grid(row=1, column=0,
                                                                                                  sticky='e')
        contactNameEnt = Entry(modifyFrame, width=20)
        contactNameEnt.insert(END, selectedSupp_contact)
        contactNameEnt.grid(row=1, column=1, sticky='w')

        Label(modifyFrame, text="เบอร์โทรศัพท์:", fg="black", bg="white", font="verdana 25").grid(row=2, column=0,
                                                                                                  sticky='e')
        phoneEnt = Entry(modifyFrame, width=20)
        phoneEnt.insert(END, selectedSupp_phone)
        phoneEnt.grid(row=2, column=1, sticky='w')

        Button(modifyFrame, text="แก้ไขผู้ค้าส่ง", fg="black", bg="yellow", borderless=1, font="verdana 25 bold",
               command=ModifySupplier).grid(row=3, columnspan=2)
...
def purchaseManagementClicked():
    ...
    # header
    Label(top, text="เพิ่ม/แก้ไขข้อมูลการสั่งซื้อ", fg="black", bg="white", font="verdana 35 bold").grid(row=0,
                                                                                                         column=0,
                                                                                                         sticky='s')

    purchaseTree = ttk.Treeview(middle)
    purchaseTree.column("#0", width=0, stretch=NO)
    purchaseTree["columns"] = (
    "รหัสสั่งซื้อ", "วันที่สั่งซื้อ", "ชื่อผู้สั่งซื้อ", "ชื่อผู้ค้าส่ง", "รายการสินค้า", "ปริมาณการสั่งซื้อ",
    "ราคาซื้อต่อหน่วย", "ค่าใช้จ่าย", "สถานะ")
    ...
    # Add Button
    Button(middle, text="สร้างรายการสั่งซื้อใหม่", fg="black", bg="green", borderless=1,
           command=addPurchaseClicked).grid(row=0, column=0, sticky='ne', padx=15, pady=170)

    # Modify Button
    Button(middle, text="แก้ไขคำสั่งซื้อ", fg="black", bg="yellow", borderless=1, command=modifyPurchaseClicked).grid(
        row=0, column=0, sticky='se', padx=160, pady=170)

    # Confirm Button
    Button(middle, text="ยืนยันคำสั่งซื้อ", fg="black", bg="green", borderless=1, command=confrimPurchaseClicked).grid(
        row=0, column=0, sticky='se', padx=280, pady=170)

    # Delete Button
    Button(middle, text="ลบ/ยกเลิกคำสั่งซื้อ", fg="black", bg="red", borderless=1, command=deletePurchaseClicked).grid(
        row=0, column=0, sticky='se', padx=15, pady=170)
...
def fetchPurchaseTree():
    ...
    if res:
        for i in range(len(res)):
            ...
            if res[i][8] == 1:
                status = "สมบูรณ์"
            purchaseTree.insert("", END, values=(
            res[i][0], res[i][1], res[i][2], res[i][3], res[i][4], res[i][5], res[i][6], res[i][7], status))
...
def addPurchaseClicked():
    def AddPurchaseOrder():
        if ordererNameCombo.get() == "":
            ...
            status = 0
            cursor.execute(sql,
                           [date, ordererName, supplierName, productList, quantities, costPerUnit, totalCost, status])
            conn.commit()
            fetchPurchaseTree()
    ...
    def fetchinfoTree():
        ...
        for i in range(len(orderProdlist)):
            infoTree.insert("", END,
                            values=(orderProdlist[i][0], orderProdlist[i][1], orderProdlist[i][2], orderProdlist[i][3]))
    ...
    def addProductlist():
        ...
        Label(top, text="ราคาซื้อต่อหน่วย:", fg="black", bg="white", font="verdana 25").grid(row=0, column=0,
                                                                                             sticky='e')
        costEnt = Entry(top, width=20)
        costEnt.grid(row=0, column=1, sticky='w')

        Label(top, text="ปริมาณที่สั่งซื้อ:", fg="black", bg="white", font="verdana 25").grid(row=1, column=0,
                                                                                              sticky='e')
        ...
        Button(bot, text="เพิ่มรายการสินค้า", fg="black", bg="green", borderless=1,
               command=addToProductListClicked).grid(row=1, column=0, sticky='e', padx=120)

        Button(bot, text="ยืนยัน", fg="black", bg="green", borderless=1, command=confirmClicked).grid(row=1, column=0,
                                                                                                      sticky='e',
                                                                                                      padx=20)
    ...
    Label(addPurchaseFrame, text="วันที่สั่งซื้อ", fg="black", bg="white", font="verdana 25").grid(row=0, column=0,
                                                                                                   sticky='e')
    ...
    Label(addPurchaseFrame, text="ผู้สั่งซื้อ:", fg="black", bg="white", font="verdana 25").grid(row=1, column=0,
                                                                                                 sticky='e')
    ordererNameCombo = ttk.Combobox(addPurchaseFrame, values=names, state="readonly")
    ordererNameCombo.grid(row=1, column=1, sticky='w')

    Label(addPurchaseFrame, text="ผู้ค้าส่ง:", fg="black", bg="white", font="verdana 25").grid(row=2, column=0,
                                                                                               sticky='e')
    supplierCombo = ttk.Combobox(addPurchaseFrame, values=suppliers, state="readonly")
    supplierCombo.grid(row=2, column=1, sticky='w')

    Label(addPurchaseFrame, text="รายการสินค้า:", fg="black", bg="white", font="verdana 25").grid(row=3, column=0,
                                                                                                  sticky='e')
    Button(addPurchaseFrame, text="เลือกรายการสินค้า", fg="white", bg="gray", font="verdana 15", borderless=1,
           command=addProductlist).grid(row=3, column=1, sticky='w', padx=5)
    ...
    totalLabel = Label(addPurchaseFrame, text="ค่าใช้จ่าย: " + str(totalCost) + " บาท", fg="red", bg="white",
                       font="verdana 20 bold")
    totalLabel.grid(row=5, columnspan=2)

    Button(addPurchaseFrame, text="เพิ่มรายการสั่งซื้อ", fg="black", bg="green", borderless=1, font="verdana 25 bold",
           command=AddPurchaseOrder).grid(row=6, columnspan=2)
...
def modifyPurchaseClicked():
        ...
        if infoTree.get_children() == ():
            ...
            for i in range(len(productsName)):
                if i == len(productsName) - 1:
                    ...
                    costPerUnit += costs[i] + "\n"
            totalCost = getTotal()
            cursor.execute(sql, [date, ordererName, supplierName, productList, quantitiesStr, costPerUnit, totalCost,
                                 oldPurchase[0]])
            conn.commit()
            fetchPurchaseTree()
    ...
    def modifyProductlist():
        if infoTree.focus() == "":
            ...
            Label(modifyProductListFrame, text="ราคาซื้อต่อหน่วย:", fg="black", bg="white", font="verdana 25").grid(
                row=0, column=0, sticky='e')
            costEnt = Entry(modifyProductListFrame, width=20)
            costEnt.insert(END, selectedcost)
            costEnt.grid(row=0, column=1, sticky='w')

            Label(modifyProductListFrame, text="ปริมาณที่สั่งซื้อ:", fg="black", bg="white", font="verdana 25").grid(
                row=1, column=0, sticky='e')
            orderQuantityEnt = Entry(modifyProductListFrame, width=20)
            orderQuantityEnt.insert(END, selectedquan)
            orderQuantityEnt.grid(row=1, column=1, sticky='w')

            Button(modifyProductListFrame, text="แก้ไขรายการสินค้า", fg="black", bg="green", borderless=1,
                   command=modifyProductListClicked).grid(row=2, columnspan=2, sticky='e', padx=120)

            Button(modifyProductListFrame, text="ลบสินค้า", fg="black", bg="green", borderless=1,
                   command=deleteProductListClicked).grid(row=2, columnspan=2, sticky='e', padx=20)
    ...
    if purchaseTree.focus() == "":
        ...
        Label(modifyPurchaseFrame, text="วันที่สั่งซื้อ", fg="black", bg="white", font="verdana 25").grid(row=0,
                                                                                                          column=0,
                                                                                                          sticky='e')
        ...
        Label(modifyPurchaseFrame, text="ผู้สั่งซื้อ:", fg="black", bg="white", font="verdana 25").grid(row=1, column=0,
                                                                                                        sticky='e')
        ...
        Label(modifyPurchaseFrame, text="ผู้ค้าส่ง:", fg="black", bg="white", font="verdana 25").grid(row=2, column=0,
                                                                                                      sticky='e')
        ...
        Label(modifyPurchaseFrame, text="รายการสินค้า:", fg="black", bg="white", font="verdana 25").grid(row=3,
                                                                                                         column=0,
                                                                                                         sticky='e')
        Button(modifyPurchaseFrame, text="เพิ่มรายการสินค้า", fg="white", bg="gray", font="verdana 15", borderless=1,
               command=addProductlist).grid(row=3, column=1, sticky='w', padx=5)
        Button(modifyPurchaseFrame, text="แก้ไขสินค้า", fg="white", bg="gray", font="verdana 15", borderless=1,
               command=modifyProductlist).grid(row=3, column=1, sticky='e', padx=10)
        ...
        totalLabel = Label(modifyPurchaseFrame, text="ค่าใช้จ่าย: " + str(totalCost) + " บาท", fg="red", bg="white",
                           font="verdana 20 bold")
        totalLabel.grid(row=5, columnspan=2)

        Button(modifyPurchaseFrame, text="แก้ไขรายการสั่งซื้อ", fg="black", bg="green", borderless=1,
               font="verdana 25 bold", command=ModifyPurchaseOrder).grid(row=6, columnspan=2)
